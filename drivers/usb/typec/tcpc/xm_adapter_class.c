		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	int sscanf_ret;
	const size_t alloc_size = sizeof(adapter_dev->vdm_store_buf);
	size_t token_len = 0;
	size_t max_hex_chars = alloc_size * 2;
	size_t i;

	mutex_lock(&adapter_dev->ops_lock);
	data = adapter_dev->vdm_store_buf;
	...
	while (token_len > 0 && isspace((unsigned char)buffer[0])) {
		memmove(buffer, buffer + 1, token_len);
	...
	if (token_len > 0) {
		if (!strncasecmp(buffer, "none", 4) ||
			...
			if (adapter_dev->ops && adapter_dev->ops->request_vdm_cmd)
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (adapter_dev->adapter_id == 0 &&
	    adapter_dev->adapter_svid == 0 &&
	    adapter_dev->verify_process == 0) {
		class_info("%s: adapter maybe removed, force set verifyed to 0\n", __func__);
		val = 0;
	}
