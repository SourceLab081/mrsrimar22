...
#define TCPC_CORE_VERSION		"2.0.19_G"
				   ...
				   const char *buf, size_t count)
{
	...
	case TCPC_DESC_ROLE_DEF:
		...
		if (ret < 0) {
			dev_err(dev, "%s: get parameters fail\n", __func__);
			return -EINVAL;
		}
...
void tcpc_dev_put(struct tcpc_device *tcpc)
{
	if (!tcpc)
		return;
	put_device(&tcpc->dev);
...
static void tcpc_device_release(struct device *dev)
{
	...
	/* Un-init mutex */
	mutex_destroy(&tcpc->mr_lock);
	mutex_destroy(&tcpc->timer_lock);
	mutex_destroy(&tcpc->typec_lock);
	mutex_destroy(&tcpc->access_lock);
}
	...
	struct tcpc_desc *tcpc_desc, struct tcpc_ops *ops, void *drv_data)
{
	...
	ret = device_init_wakeup(&tcpc->dev, true);
	if (ret) {
		pr_err("%s: device init wake failed: %d\n", __func__, ret);
		goto err_dev;
	}
	...
	if (!tcpc->attach_wake_lock) {
		pr_err("%s: register attach wake lock failed\n", __func__);
		ret = -ENOMEM;
		goto err_wake;
	}
	...
	if (!tcpc->detach_wake_lock) {
		pr_err("%s: register detach wake lock failed\n", __func__);
		ret = -ENOMEM;
		goto err_attach_wl;
	}

	ret = tcpci_timer_init(tcpc);
	if (ret < 0) {
...
#ifdef CONFIG_USB_POWER_DELIVERY
	ret = pd_core_init(tcpc);
	if (ret < 0) {
		pr_err("%s: pd_core_init failed: %d\n", __func__, ret);
		goto err_timer;
	}
#endif /* CONFIG_USB_POWER_DELIVERY */
	...
	if (ret < 0) {
		pr_err("%s: dual role usb init fail: %d\n", __func__, ret);
		goto err_pd_core;
	}
	pr_info("%s: dual role usb init OK\n", __func__);
...
err_detach_wl:
	...
	tcpc->detach_wake_lock = NULL;
err_attach_wl:
	wakeup_source_unregister(tcpc->attach_wake_lock);
	tcpc->attach_wake_lock = NULL;
err_wake:
	device_init_wakeup(&tcpc->dev, false);
err_dev:
...
err_free:
	...
	pr_err("%s: tcpc device register failed\n", __func__);
	return ERR_PTR(ret);
}
...
static void bat_update_work_func(struct work_struct *work)
{
	...
	if (ret == 0) {
		TCPC_INFO("battery update soc = %d\n", value.intval);
		tcpc->bat_soc = value.intval;
	} else {
		TCPC_ERR("get battery capacity fail\n");
	}
...
static void tcpc_event_init_work(struct work_struct *work)
{
	...
	tcpci_lock_typec(tcpc);
	if (tcpci_event_init(tcpc) < 0) {
		TCPC_ERR("event init failed, abort PD init\n");
		tcpci_unlock_typec(tcpc);
		return;
	}
	...
	if (tcpc->usb_psy) {
		tcpc->usb_psy_got = true;
	} else {
		tcpc->usb_psy_got = false;
		TCPC_ERR("get usb psy fail\n");
		tcpci_event_deinit(tcpc);
		tcpci_unlock_typec(tcpc);
		return;
	}
	...
	tcpc->bat_work_inited = true;

	tcpc->bat_psy = power_supply_get_by_name("battery");
	if (!tcpc->bat_psy) {
		TCPC_ERR("get battery psy fail\n");
		tcpc->bat_nb_registered = false;
	} else {
		tcpc->bat_nb.notifier_call = bat_nb_call_func;
		tcpc->bat_nb.priority = 0;
		retval = power_supply_reg_notifier(&tcpc->bat_nb);
		if (retval < 0) {
			TCPC_ERR("register power supply notifier fail: %d\n",
				 retval);
			tcpc->bat_nb_registered = false;
		} else {
			tcpc->bat_nb_registered = true;
		}
	}
#endif /* CONFIG_USB_PD_REV30 */
#endif /* CONFIG_USB_POWER_DELIVERY */
...
#ifdef CONFIG_USB_PD_REV30
	if (tcpc->bat_work_inited)
		cancel_delayed_work_sync(&tcpc->bat_update_work);

	if (tcpc->bat_nb_registered)
		power_supply_unreg_notifier(&tcpc->bat_nb);

	if (tcpc->bat_psy) {
		power_supply_put(tcpc->bat_psy);
		tcpc->bat_psy = NULL;
		tcpc->bat_nb_registered = false;
		tcpc->bat_work_inited = false;
	}

#ifdef CONFIG_USB_PD_WAIT_BC12
	if (tcpc->usb_psy) {
		power_supply_put(tcpc->usb_psy);
		tcpc->usb_psy = NULL;
		tcpc->usb_psy_got = false;
	}
#endif /* CONFIG_USB_PD_WAIT_BC12 */
...
static int __init tcpc_class_init(void)
{
	...
	if (IS_ERR(tcpc_class)) {
		pr_info("%s: Unable to create tcpc class, errno = %ld\n",
			__func__, PTR_ERR(tcpc_class));
...
/* Release Version
 * 2.0.19_G
 * (1) revise module load handling
 * (2) stub out late_sync into submodule
 * (3) fix PM not working silently due to no wake capable device during wakeup_source registration
 *
 * 2.0.18_G
 * (1) Fix typos
