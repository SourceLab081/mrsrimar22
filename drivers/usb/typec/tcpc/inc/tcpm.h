...
#define TCP_NOTIFY_TYPE_MODE	(1U << TCP_NOTIFY_IDX_MODE)
#define TCP_NOTIFY_TYPE_VBUS	(1U << TCP_NOTIFY_IDX_VBUS)
#define TCP_NOTIFY_TYPE_USB	(1U << TCP_NOTIFY_IDX_USB)
#define TCP_NOTIFY_TYPE_MISC	(1U << TCP_NOTIFY_IDX_MISC)

#define TCP_NOTIFY_TYPE_ALL	((1U << TCP_NOTIFY_IDX_NR) - 1)
...
enum {
	...
	TCP_DP_UFP_U_POWER_LOW = (1U << 2),
	TCP_DP_UFP_U_ENABLED = (1U << 3),
	TCP_DP_UFP_U_MF_PREFER = (1U << 4),
	TCP_DP_UFP_U_USB_CONFIG = (1U << 5),
	TCP_DP_UFP_U_EXIT_MODE = (1U << 6),
};
...
#define DPM_FLAGS_PARTNER_DR_POWER		(1U << 0)
#define DPM_FLAGS_PARTNER_DR_DATA		(1U << 1)
#define DPM_FLAGS_PARTNER_EXTPOWER		(1U << 2)
#define DPM_FLAGS_PARTNER_USB_COMM		(1U << 3)
#define DPM_FLAGS_PARTNER_USB_SUSPEND		(1U << 4)
#define DPM_FLAGS_PARTNER_HIGH_CAP		(1U << 5)

#define DPM_FLAGS_PARTNER_MISMATCH		(1U << 7)
#define DPM_FLAGS_PARTNER_GIVE_BACK		(1U << 8)
#define DPM_FLAGS_PARTNER_NO_SUSPEND		(1U << 9)
...
#define DPM_CAP_LOCAL_DR_POWER			(1U << 0)
#define DPM_CAP_LOCAL_DR_DATA			(1U << 1)
#define DPM_CAP_LOCAL_EXT_POWER			(1U << 2)
#define DPM_CAP_LOCAL_USB_COMM			(1U << 3)
#define DPM_CAP_LOCAL_USB_SUSPEND		(1U << 4)
#define DPM_CAP_LOCAL_HIGH_CAP			(1U << 5)
#define DPM_CAP_LOCAL_GIVE_BACK			(1U << 6)
#define DPM_CAP_LOCAL_NO_SUSPEND		(1U << 7)
#define DPM_CAP_LOCAL_VCONN_SUPPLY		(1U << 8)

#define DPM_CAP_ATTEMP_ENTER_DC_MODE		(1U << 11)
#define DPM_CAP_ATTEMP_DISCOVER_CABLE_DFP	(1U << 12)
#define DPM_CAP_ATTEMP_ENTER_DP_MODE		(1U << 13)
#define DPM_CAP_ATTEMP_DISCOVER_CABLE		(1U << 14)
#define DPM_CAP_ATTEMP_DISCOVER_ID		(1U << 15)
#define DPM_CAP_ATTEMP_DISCOVER_SVID		(1U << 16)
...
#define DPM_CAP_PR_SWAP_REJECT_AS_SRC		(1U << 20)
#define DPM_CAP_PR_SWAP_REJECT_AS_SNK		(1U << 21)
#define DPM_CAP_PR_SWAP_CHECK_GP_SRC		(1U << 22)
#define DPM_CAP_PR_SWAP_CHECK_GP_SNK		(1U << 23)
...
#define DPM_CAP_DR_SWAP_REJECT_AS_DFP		(1U << 24)
#define DPM_CAP_DR_SWAP_REJECT_AS_UFP		(1U << 25)

#define DPM_CAP_DP_PREFER_MF		(1U << 29)
...
enum tcpm_power_cap_apdo_type {
	TCPM_POWER_CAP_APDO_TYPE_PPS = (1U << 0),

	TCPM_POWER_CAP_APDO_TYPE_PPS_CF = (1U << 7),
};
...
typedef int (*tcp_dpm_event_cb)(struct tcpc_device *tcpc, int ret, struct tcp_dpm_event *event);
...
enum dpm_charging_policy {
	...
	DPM_CHARGING_POLICY_IGNORE_MISMATCH_CURR = (1U << 4),
	DPM_CHARGING_POLICY_PREFER_LOW_VOLTAGE = (1U << 5),
	DPM_CHARGING_POLICY_PREFER_HIGH_VOLTAGE = (1U << 6),
...
extern void tcpc_dev_put(struct tcpc_device *tcpc);
...
extern int tcpm_set_apdo_charging_policy(struct tcpc_device *tcpc,
...
static inline int tcpm_inquire_pd_partner_modes(struct tcpc_device *tcpc,
	uint16_t svid, struct tcpm_mode_list *list)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline bool tcpm_extract_power_cap_list(struct tcpm_power_cap *cap,
	struct tcpm_power_cap_list *cap_list)
{
	return false;
}
...
static inline int tcpm_inquire_select_source_cap(struct tcpc_device *tcpc,
	struct tcpm_power_cap_val *cap_val)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_dpm_pd_cable_soft_reset(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_dpm_vdm_discover_cable(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_dpm_vdm_discover_id(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}

static inline int tcpm_dpm_vdm_discover_svid(struct tcpc_device *tcpc,
	const struct tcp_dpm_event_cb_data *data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
...
static inline int tcpm_set_apdo_charging_policy(struct tcpc_device *tcpc,
	uint8_t policy, int mv, int ma,
	const struct tcp_dpm_event_cb_data *data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
