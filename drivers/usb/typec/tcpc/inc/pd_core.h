...
#define PDO_TYPE_APDO		(3U << 30)
#define PDO_TYPE_MASK		(3U << 30)

#define PDO_FIXED_DUAL_ROLE	(1U << 29) /* Dual role device */
#define PDO_FIXED_SUSPEND	(1U << 28) /* USB Suspend supported (SRC) */
#define PDO_FIXED_HIGH_CAP	(1U << 28) /* Higher Capability (SNK) */
#define PDO_FIXED_EXTERNAL	(1U << 27) /* Externally powered */
#define PDO_FIXED_COMM_CAP	(1U << 26) /* USB Communications Capable */
#define PDO_FIXED_DATA_SWAP	(1U << 25) /* Data role swap command supported */
...
#define APDO_TYPE_MASK	(3U << 28)
#define APDO_TYPE_PPS	(0U << 28)

#define APDO_TYPE(raw)	((raw) & APDO_TYPE_MASK)
#define APDO_TYPE_VAL(raw)	(APDO_TYPE(raw) >> 28)

#define APDO_PPS_CURR_FOLDBACK	(1U << 26)
...
#define APDO_PPS(min_mv, max_mv, ma, flags)	\
	(APDO_PPS_MIN_VOLT(min_mv) | APDO_PPS_MAX_VOLT(max_mv) | \
	 APDO_PPS_CURR(ma) | flags | PDO_TYPE_APDO | APDO_TYPE_PPS)
...
#define RDO_COMM_CAP	(1U << 25)
#define RDO_NO_SUSPEND	(1U << 24)
#define RDO_EXTEND_MESSAGE	(1U << 23)
#define RDO_CURR_FOLDBACK	(1U << 22)
...
#define BDO_MODE_TEST_DATA	(8U << 28)
...
#define VDO_IDH(usbh, usbd, ptype, ptype_dfp, is_modal, vid)	\
	(((uint32_t)(usbh) << 31) | ((uint32_t)(usbd) << 30) | \
	(((ptype) & 0x7) << 27) | (((ptype_dfp) & 0x07) << 23) | \
...
#define PD_IDH_MODAL_SUPPORT	(1U << 26)
...
#define PD_DP_PIN_CAPS(x)	((((x) >> 6) & 0x1) ? \
	(((x) >> 16) & 0x3f) : (((x) >> 8) & 0x3f))
...
#define DPSTS_DFP_D_CONNECTED	(1U << 0)
#define DPSTS_UFP_D_CONNECTED	(1U << 1)
#define DPSTS_BOTH_CONNECTED	(DPSTS_DFP_D_CONNECTED | DPSTS_UFP_D_CONNECTED)

/* UFP_U only */
#define DPSTS_DP_ENABLED	(1U << 3)
#define DPSTS_DP_MF_PREF	(1U << 4)
#define DPSTS_DP_USB_CONFIG	(1U << 5)
#define DPSTS_DP_EXIT_ALT_MODE	(1U << 6)

/* UFP_D only */
#define DPSTS_DP_HPD_STATUS	(1U << 7)
#define DPSTS_DP_HPD_IRQ	(1U << 8)
...
#define DP_PIN_ASSIGN_SUPPORT_A	(1U << 0)
#define DP_PIN_ASSIGN_SUPPORT_B	(1U << 1)
#define DP_PIN_ASSIGN_SUPPORT_C	(1U << 2)
#define DP_PIN_ASSIGN_SUPPORT_D	(1U << 3)
#define DP_PIN_ASSIGN_SUPPORT_E	(1U << 4)
#define DP_PIN_ASSIGN_SUPPORT_F	(1U << 5)
...
extern void pd_core_deinit(struct tcpc_device *tcpc);
...
uint32_t pd_reset_pdo_power(struct tcpc_device *tcpc, uint32_t pdo, uint32_t imax);
...
int pd_send_data_msg(struct pd_port *pd_port, uint8_t sop_type,
...
#ifdef CONFIG_USB_PD_REV30
int pd_send_ext_msg(struct pd_port *pd_port, uint8_t sop_type,
	uint8_t msg, bool request,
