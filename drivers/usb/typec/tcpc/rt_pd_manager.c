...
static inline int smblib_get_bbc_prop(struct rt_pd_manager_data *rpmd,
...
static void usb_dwork_handler(struct work_struct *work)
{
	...
	case DR_MAX:
		...
		break;
	case DR_DEVICE:
		ret = smblib_get_bbc_prop(rpmd, POWER_SUPPLY_PROP_REAL_TYPE, &val);
				    ...
				    int mv, int ma, uint8_t type)
{
	...
	/* HTH-Fix race between USB PD set prop and batterysecret store fail */
	...
	case TCP_VBUS_CTRL_PD_REQUEST:
		/* Only store valid PD contract values, not standby/reset events */
		rpmd->sink_mv_pd = mv;
		rpmd->sink_ma_pd = ma;
		set_bit(0, &sel);
		set_bit(1, &sel);
...
static int pd_tcp_notifier_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (new_state == TYPEC_UNATTACHED) {
			...
			rpmd->partner = NULL;
			/* Reset PD contract values to avoid stale data on next plug-in */
			rpmd->sink_mv_pd = 0;
			rpmd->sink_ma_pd = 0;
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_PE_READY_SNK_PD30:
			...
			val.intval = noti->pd_state.connected == PD_CONNECT_PE_READY_SNK_APDO ?
					POWER_SUPPLY_PD_PPS_ACTIVE : POWER_SUPPLY_PD_ACTIVE;
			smblib_set_prop(rpmd, POWER_SUPPLY_PROP_PD_ACTIVE, &val);
			pd_sink_set_vol_and_cur(rpmd,
					rpmd->sink_mv_pd,
					rpmd->sink_ma_pd,
...
static int rt_pd_manager_probe(struct platform_device *pdev)
{
	...
	chip = rt1711_chip_get(RT1711H_NAME);
	if (!chip) {
		ret = -EPROBE_DEFER;
		dev_err(&pdev->dev, "%s: rt1711_chip not ready (%d/%d)\n",
				__func__, probe_cnt, PROBE_CNT_MAX);
		if (probe_cnt >= PROBE_CNT_MAX)
			ret = -ENODEV;
		return ret;
	...
	if (!rt1711_chip_is_probed(chip)) {
		rt1711_chip_put(chip);
		ret = -EPROBE_DEFER;
		dev_err(&pdev->dev, "%s: rt1711_chip not probed yet (%d/%d)\n",
				__func__, probe_cnt, PROBE_CNT_MAX);
		if (probe_cnt >= PROBE_CNT_MAX)
			ret = -ENODEV;
		return ret;
	...
	if (!rpmd->bbc_psy) {
		dev_err(&pdev->dev, "%s: get bbc psy fail\n", __func__);
		ret = -EPROBE_DEFER;
		if (probe_cnt >= PROBE_CNT_MAX)
			goto out;
		else
	...
	/* This is the minimum arm IRQ point after consumer is registered. */
	ret = tcpci_late_sync_enable(rpmd->tcpc);
	if (ret < 0) {
		dev_err(&pdev->dev, "%s: late sync enable fail(%d)\n", __func__, ret);
		ret = -EPROBE_DEFER;
		if (probe_cnt >= PROBE_CNT_MAX)
			goto out;
		else
...
err_late_sync:
	unregister_tcp_dev_notifier(rpmd->tcpc, &rpmd->pd_nb, TCP_NOTIFY_TYPE_ALL);
err_reg_tcpc_notifier:
	typec_unregister_port(rpmd->typec_port);
err_init_typec:
err_init_extcon:
	if (rpmd->tcpc)
		tcpc_dev_put(rpmd->tcpc);
err_get_tcpc_dev:
	if (rpmd->bbc_psy)
