...
int tcpci_timer_init(struct tcpc_device *tcpc)
{
	...
	pr_info("%s: PD Timer number = %d\n", __func__, PD_TIMER_NR);
	...
	if (IS_ERR(tcpc->timer_task)) {
		int err = PTR_ERR(tcpc->timer_task);

		pr_err("%s: failed to create timer kthread (%d)\n", __func__, err);
		tcpc->timer_task = NULL;
		return err;
	}
	...
	tcpc->wakeup_wake_lock = wakeup_source_register(&tcpc->dev,
				    "tcpc_wakeup_wake_lock");
	if (!tcpc->wakeup_wake_lock) {
...
void tcpci_timer_deinit(struct tcpc_device *tcpc)
{
	int i;
	...
	for (i = 0; i < PD_TIMER_NR; i++)
		hrtimer_cancel(&tcpc->tcpc_timer[i]);
