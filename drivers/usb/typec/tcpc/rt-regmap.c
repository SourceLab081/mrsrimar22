				     ...
				     unsigned char mode)
{
	...
	if (prev_io_blk == RT_IO_BLK_CACHE) {
		dev_info(&rd->dev,
			"%s: invalidating stale cache (was BLK_CACHE)\n",
			__func__);
		...
		size_t count, loff_t *ppos)
{
	...
	*ppos = 0;
	res = simple_write_to_buffer(lbuf, sizeof(lbuf) - 1, ppos, ubuf,
				     count);
	if (res <= 0)
		return -EFAULT;
	count = res;
...
{
	/* read-write nodes */
	RT_CREATE_GENERAL_FILE(RT_DBG_REG_ADDR,    "reg_addr",    0644);
	RT_CREATE_GENERAL_FILE(RT_DBG_DATA,        "data",        0644);
	RT_CREATE_GENERAL_FILE(RT_DBG_SYNC,        "sync",        0644);
	RT_CREATE_GENERAL_FILE(RT_DBG_ERROR,       "Error",       0644);
	RT_CREATE_GENERAL_FILE(RT_DBG_BLOCK,       "block",       0644);
	RT_CREATE_GENERAL_FILE(RT_DBG_SIZE,        "size",        0644);
	RT_CREATE_GENERAL_FILE(RT_DBG_IO_LOG,      "io_log",      0644);
	RT_CREATE_GENERAL_FILE(RT_DBG_CACHE_MODE,  "cache_mode",  0644);
	/* read-only nodes */
 ...
 *	      add cancel_delayed_work_sync() before kfree
 *	fix potential deadlock in rt_regmap_set_cache_mode -
 *	      enforce consistent lock order: semaphore -> write_mode_lock
 *	fix BLK_ALL silent data discard - return -EPERM not 0
 *	fix stale cache after BLK_CACHE - invalidate on mode switch
 *	fix partial dirty-bit clearing in cache_sync/write_back -
 *	      clear per-byte after each successful chip write
 *	fix debugfs permissions - 0644 for writable nodes
 *	remove dead code in rt_cache_clrlasterror
 *	fix false error return when IO log buffer overflows
 *	fix eachreg_read / rt_show_regs / general_read race on
 *	      rd->regval - use local stack buffer instead
