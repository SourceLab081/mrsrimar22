...
#define BATT_MAX_CHG_VOLT	4450 //new requirement from xiaomi hw, CP should config 4460
...
#define pdpm_log(fmt, ...)								\
do {											\
	if (pdpm && __ratelimit(&pdpm->rl_slots[_PDPM_ID() & (PDPM_RL_SLOTS - 1)]))	\
...
static int usbpd_pm_update_sw_status(struct usbpd_pm *pdpm)
{
	...
	ret = charger_dev_get_adc(pdpm->sw_chg, PCA_ADCCHAN_IBUS, &ibus,
			&ibus);
	if (ret < 0) {
		pdpm_err("get swchg ibus fail(%d)\n", ret);
		return ret;
	}
...
static bool pd_disable_cp_by_jeita_status(struct usbpd_pm *pdpm)
{
	...
	if (batt_temp >= warm_thres && !pdpm->jeita_triggered) {
		...
		return true;
			...
			(batt_temp >= (cool_thres + JEITA_HYSTERESIS)) && pdpm->jeita_triggered) {
...
static bool usbpd_get_pps_status(struct usbpd_pm *pdpm)
{
	...
	while (1) {
		ret = tcpm_inquire_pd_source_apdo(pdpm->tcpc,
				...
				&cap_idx, &apdo_cap);
		if (ret != TCP_DPM_RET_SUCCESS) {
			pdpm_err("inquire pd apdo fail(%d)\n", ret);
			break;
		}
	...
	for (i = 0; i < 7; i++) {
		...
		if (pdpm->pdo[i].apdo == true) {
			if (pdpm->pdo[i].max_voltage >= pdpm->apdo_max_volt
				...
				pdpm_log("[SC manager] vola %d curr %d\n",
...
static void usbpd_pm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	if (!pdpm->pd) {
		...
		if (!pdpm->pd) {
			pdpm_err("couldn't get usbpd device\n");
			return;
		}
	}
	...
	if (ret) {
		pdpm_err("Failed to fetch pdo info\n");
		return;
	}
...
static void usbpd_update_pps_status(struct usbpd_pm *pdpm)
{
	...
	if (!ret) {
		...
		pdpm_log("adapter_volt: %d, adapter_current: %d\n",
...
/*static void usb_psy_pd_active_update(struct usbpd_pm *pdpm)
{
	...
	if (ret) {
		pdpm_err("Failed to get usb pd active state\n");
		return;
	}
...
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
	...
	if (pdpm->cp.vbat_volt > pm_config.bat_volt_lp_lmt)
		...
		step_vbat = pm_config.fc2_steps;
	pdpm_log("vbat: %d, lmt: %d, step: %d\n", pdpm->cp.vbat_volt, pm_config.bat_volt_lp_lmt, step_vbat);
	...
	if (pdpm->cp.ibat_curr < fcc_limit)
		...
		step_ibat = -pm_config.fc2_steps;
	pdpm_log("ibat: %d, lmt: %d, step: %d\n", pdpm->cp.ibat_curr, fcc_limit, step_ibat);
	...
	if (pdpm->cp.bat_therm_fault) { /* battery overheat, stop charge*/
		...
		pdpm_log("bat_ocp_fault: %d, bus_ocp_fault: %d, bat_ovp_fault: %d, bus_ovp_fault: %d\n",
				pdpm->cp.bat_ocp_fault,
				pdpm->cp.bus_ocp_fault,
				pdpm->cp.bat_ovp_fault,
	...
	volt_ceil = (pdpm->apdo_max_volt > 10000) ? pdpm->apdo_max_volt - 1000 : pdpm->apdo_max_volt;
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	case PD_PM_STATE_ENTRY:
		...
		if (pdpm->cp.vbat_volt < pm_config.min_vbat_for_cp) {
			...
			pdpm_log("pm_sm batt_volt: %d too high for cp, switch main charger\n", pdpm->cp.vbat_volt);
			usbpd_pm_move_state(pdpm, PD_PM_STATE_FC2_EXIT);
		} else if (thermal_level >= MAX_THERMAL_LEVEL_FOR_CP || pdpm->is_temp_out_fc2_range) {
			pdpm_log("system thermal level too high or batt temp is out of fc2 range, waiting...\n");
		} else {
	...
	case PD_PM_STATE_FC2_ENTRY_2:
		pdpm_log("tune_vbus_retry %d, vbus_low: %d, vbus_high: %d\n", pdpm->tune_vbus_retry, pdpm->cp.vbus_error_low, pdpm->cp.vbus_error_high);
		...
		if (pdpm->tune_vbus_retry > 30) {
			pdpm_log("Failed to tune adapter volt into valid range, charge with switching charger, will try to recover\n");
			usbpd_pm_move_state(pdpm, PD_PM_STATE_FC2_EXIT);
	...
	case PD_PM_STATE_FC2_TUNE:
		...
		if (ret == PM_ALGO_RET_THERM_FAULT) {
			...
			pdpm_log("Move to switch charging, will try to recover flash charging: %d\n", ret);
			pdpm->recover = true;
			usbpd_pm_move_state(pdpm, PD_PM_STATE_FC2_EXIT);
			break;
		} else if (ret == PM_ALGO_RET_TAPER_DONE) {
			pdpm_log("Move to switch charging, taper is done: %d\n", ret);
			usbpd_pm_move_state(pdpm, PD_PM_STATE_FC2_EXIT);
			break;
		} else {
			usbpd_select_pdo(pdpm, pdpm->request_voltage, pdpm->request_current);
	...
	case PD_PM_STATE_FC2_EXIT:
		...
		pdpm_log(">>>sw state %d %d\n", pdpm->stop_sw, pdpm->sw.charge_enabled);
		if (pdpm->stop_sw && pdpm->sw.charge_enabled)
			usbpd_pm_enable_sw(pdpm, false);
		else if (!pdpm->stop_sw && !pdpm->sw.charge_enabled)
			...
			unsigned long event, void *data)
{
	...
	if (psy == pdpm->cp_psy || psy == pdpm->usb_psy) {
		...
		pdpm_log("[SC manager] >>>pdpm->psy_change_running: %d\n", pdpm->psy_change_running);
...
static int usbpd_pm_probe(struct platform_device *pdev)
{
	...
	if (!pdpm->cp_psy ||
			(pm_config.cp_sec_enable && !pdpm->cp_sec_psy) ||
			!pdpm->usb_psy ||
			((NOPMI_CHARGER_IC_MAXIM != nopmi_get_charger_ic_type()) && !pdpm->tcpc)) {
		ret = -EPROBE_DEFER;
		goto err_psy;
	}
