...
/*
 * onewire_gpio.c - Bit-bang 1-Wire bus driver via direct GPIO register access
 *
...
#define ow_err(fmt, ...)	pr_err(fmt, ##__VA_ARGS__)
#define ow_info(fmt, ...)	pr_info(fmt, ##__VA_ARGS__)
#define ow_dbg(fmt, ...)	pr_debug(fmt, ##__VA_ARGS__)
...
static void ow_bus_kref_release(struct kref *ref)
{
	...
	WARN_ON(READ_ONCE(bus->priv) != NULL);
	kfree(bus);
}
 ...
 */
static unsigned char ow_read_bit(struct onewire_gpio_data *od)
{
	unsigned long flags;
	unsigned int val;
 ...
 */
static void ow_write_bit(struct onewire_gpio_data *od, char bitval)
{
	unsigned long flags;
 ...
 */
static unsigned char ow_ops_reset(struct onewire_bus *bus)
{
	...
	ow_pin_input(od);
	udelay(7);
	presence = (unsigned char)ow_pin_read(od);
	local_irq_restore(flags);
	udelay(50);
...
static unsigned char ow_ops_read_byte(struct onewire_bus *bus)
{
	struct onewire_gpio_data *od = READ_ONCE(bus->priv);
	unsigned long flags;
...
static void ow_ops_write_byte(struct onewire_bus *bus, char val)
{
	struct onewire_gpio_data *od = READ_ONCE(bus->priv);
	unsigned long flags;
	unsigned char i;
				 ...
				 struct onewire_gpio_data *od)
{
	...
	if (ret) {
		ow_err("missing 'label' property: %d\n", ret);
		return ret;
	}
	...
	ret = of_property_read_u32_array(np, "mi,onewire-gpio-cfg-addr",
					 od->gpio_reg, 2);
	if (ret) {
		ow_err("missing mi,onewire-gpio-cfg-addr: %d\n", ret);
		return ret;
	}
...
static int onewire_gpio_pinctrl_init(struct onewire_gpio_data *od)
{
	...
	if (IS_ERR_OR_NULL(od->pinctrl_state_active)) {
		...
		ow_err("lookup onewire_active: %d\n", ret);
		return ret;
	}
...
static int onewire_gpio_probe(struct platform_device *pdev)
{
	...
	if (od->ow_gpio_pinctrl) {
		ret = pinctrl_select_state(od->ow_gpio_pinctrl,
					od->pinctrl_state_active);
		if (ret) {
			ow_err("select active pinstate: %d\n", ret);
			return ret;
		}
	}

	if (!gpio_is_valid(od->ow_gpio)) {
		ow_err("invalid GPIO %d\n", od->ow_gpio);
		return -EINVAL;
	}

	ret = gpio_request(od->ow_gpio, "onewire gpio");
	if (ret) {
		ow_err("gpio_request: %d\n", ret);
		return ret;
	}
	...
	if (!od->gpio_in_out_reg) {
		ow_err("ioremap gpio_in_out_reg failed\n");
		ret = -ENOMEM;
		goto err_free_gpio;
	}
	...
	if (!od->gpio_cfg_reg) {
		ow_err("ioremap gpio_cfg_reg failed\n");
		ret = -ENOMEM;
		goto err_free_gpio;
	}

	ow_info("cfg_reg=%p in_out_reg=%p\n",
			od->gpio_cfg_reg, od->gpio_in_out_reg);

	if (!dev->parent || !dev->parent->parent) {
		ow_err("parent chain unavailable\n");
		ret = -ENODEV;
		goto err_free_gpio;
	}
	...
	if (ret) {
		ow_err("onewire_bus_register('%s'): %d\n", od->bus_label, ret);
		kfree(bus);
		goto err_remove_link;
	}
...
static int __init onewire_gpio_init(void)
{
	...
	if (onewire_major < 0) {
		ret = onewire_major;
		ow_err("register_chrdev failed: %d\n", ret);
		goto err_class;
	}

	ret = platform_driver_register(&onewire_gpio_driver);
	if (ret) {
		ow_err("platform_driver_register failed: %d\n", ret);
		goto err_chrdev;
	}

	ow_info("driver registered\n");
	return 0;
