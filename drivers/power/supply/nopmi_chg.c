		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TYPEC_CC_ORIENTATION:
		val->intval = nopmi_chg->cc_orientation + 1; /*for cit test*/
		break;
	default:
		pr_debug("prop %d not supported\n", psp);
		return -EINVAL;
...
{
	struct nopmi_chg *nopmi_chg = container_of(work, struct nopmi_chg, usb_online_work.work);
...
{
	struct nopmi_chg *nopmi_chg = container_of(work, struct nopmi_chg, cv_step_monitor_work.work);
...
static int nopmi_chg_probe(struct platform_device *pdev)
{
	...
	if (!nopmi_chg->jeita_st) {
		pr_err("Failed to allocate jeita_st\n");
		rc = -ENOMEM;
	...
	nopmi_chg->usb_online_wq = create_singlethread_workqueue("nopmi_usb_online_wq");
	...
	INIT_DELAYED_WORK(&nopmi_chg->usb_online_work, nopmi_usb_online_workfunc);
	INIT_DELAYED_WORK(&nopmi_chg->nopmi_chg_work, nopmi_chg_workfunc);
	INIT_DELAYED_WORK(&nopmi_chg->cv_step_monitor_work, nopmi_cv_step_monitor_workfunc);
	...
	if (NOPMI_CHARGER_IC_NONE != nopmi_get_charger_ic_type() &&
		NOPMI_CHARGER_IC_MAX != nopmi_get_charger_ic_type()) {
