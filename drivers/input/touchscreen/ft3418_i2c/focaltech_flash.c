...
int fts_flash_write_buf(u32 saddr, u8 *buf, u32 len, u32 delay)
{
	...
	write_ctx.written_packets = 0;
	write_ctx.start_addr = saddr;
	write_ctx.total_len = len;
	write_ctx.partial_write_detected = false;
	...
	for (i = 0; i < packet_number; i++) {
		...
		if (upg->ts_data->bus_type == BUS_TYPE_SPI_V2) {
			...
			if (ret < 0) {
				FTS_ERROR("set flash address fail at packet %u/%u, addr=0x%x",
						i, packet_number, addr);
		...
		ret = fts_write(packet_buf, packet_len + cmdlen);
		if (ret < 0) {
			FTS_ERROR("app write fail at packet %u/%u, addr=0x%x",
				i, packet_number, addr);
			write_ctx.partial_write_detected = true;
			break;
		}
		...
		for (j = 0; j < FTS_RETRIES_WRITE; j++) {
			...
			ret = fts_read(&cmd , 1, val, FTS_CMD_FLASH_STATUS_LEN);
			if (ret < 0) {
				FTS_ERROR("status read fail at packet %u, retry %u", i, j);
				continue;
			}
		...
		if (j >= FTS_RETRIES_WRITE) {
			FTS_ERROR("packet %u status confirmation timeout", i);
			write_ctx.partial_write_detected = true;
			break;
		}
	}
	...
	if (write_ctx.partial_write_detected) {
		FTS_ERROR("PARTIAL WRITE DETECTED: %u/%u packets at addr 0x%x",
				write_ctx.written_packets, write_ctx.total_packets,
				write_ctx.start_addr);
		return -EIO;
	}
	...
	FTS_DEBUG("all %u packets written, ECC: 0x%04x", write_ctx.written_packets, ecc_in_host);
	return ecc_in_host;
}
...
static int fts_param_get_ver_in_host(struct fts_upgrade *upg, u8 *ver)
{
	...
	FTS_DEBUG("fw paramcfg version offset:%x", upg->func->paramcfgveroff); *ver = upg->fw[upg->func->paramcfgveroff];
...
static int fts_param_ide_in_host(struct fts_upgrade *upg)
{
	...
	if (upg->fw_length < upg->func->paramcfgoff + FTS_FW_IDE_SIG_LEN) {
		FTS_DEBUG("fw len(%x) < paramcfg offset(%x), no IDE", upg->fw_length, upg->func->paramcfgoff + FTS_FW_IDE_SIG_LEN);
		return 0;
	}
...
static int fts_param_need_upgrade(struct fts_upgrade *upg)
{
	...
	if ((0 == ide_in_host) && (0 == ide_in_tp)) {
		...
		FTS_DEBUG("fw paramcfg version in tp:%x, host:%x", ver_in_tp, ver_in_host);
...
int fts_fwupg_exit(struct fts_ts_data *ts_data)
{
	...
	if (ts_data && ts_data->ts_workqueue) {
		...
		FTS_DEBUG("upgrade work queue cancelled");
	}
	if (fwupgrade) {
