...
static void fts_rw_op_cleanup(void)
{
	kfree_safe(rw_op.opbuf);
	rw_op.dirty = false;
}
	...
	if (RWREG_OP_READ == rw_op.type) {
		if (rw_op.len == 1) {
			...
			if (rw_op.res < 0) {
				...
				FTS_DEBUG("read 0x%02x successful", rw_op.reg);
				rw_op.res = 0;
			}
		} else {
			char reg = rw_op.reg & 0xFF;
			...
			if (rw_op.res < 0) {
				...
				rw_op.res = 0;
				rw_op.dirty = true; // Mark result available
			}
		}
	} else if (RWREG_OP_WRITE == rw_op.type) {
		if (rw_op.len == 1) {
			u8 reg = rw_op.reg & 0xFF;
			u8 val = rw_op.val & 0xFF;
		...
		if (rw_op.res < 0) {
			...
			FTS_DEBUG("Write 0x%02x successful", rw_op.reg);
			rw_op.res = 0;
		}
	}
